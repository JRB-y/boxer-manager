...
$('#boxers-index').on('click', '.open-boxer-details', function () {
  ...
  function populateModal(response) {
    ...
    $('#prenom').val(response.prenom); // $('#prenom').val(response.types);
    ...
    $('#birth_date').datepicker('setDate', day + '/' + month + '/' + year); // $('#birth_date').val();
    ...
    select.multipleSelect('setSelects', types); // $('.types').multipleSelect('setSelects', types);
